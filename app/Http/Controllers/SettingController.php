...
class SettingController extends Controller {
	...
	**/
	...
	/**  
	*  For Sold house popup
	*  @author Madasamy 
	*  @param $request
	*  Created At 2020/10/02
	**/
	public function soldHousePopup(Request $request){

		$userId = Auth::user()->userId;
		$houses = Setting::fnGetHouses($userId,$request->viewFlg);
		$houseArr = array();
	...
	*  Created At 2020/10/02
	**/
	public function soldHouseProcess(Request $request){

		$update =  Setting::fnUpdateHouseSold($request);
	...
	*/
	public function threetextpopup(Request $request) {
		$getTableFields = settingcommon::getDbFieldsforProcess();
		$tablename = $request->tablename;
		$getdetails = Setting::selectOnefieldDatas($getTableFields[$tablename]['selectfields'],
											  $getTableFields[$tablename]['commitfields'][0],
		...
		$selectfiled  = $getTableFields[$tablename]['selectfields'];
		return view('setting.threetextpopup',compact('getdetails',
														'headinglbl',
														'field1lbl',
														'field2lbl',
														'field3lbl',
	...
	**/
	...
	public function ThreeFieldaddedit(Request $request) {
		if ($request->flag == 2) {
		...
		$orderidval = Setting::Orderidgenerate($tbl_name);
 		$orderidarray['orderid'] = $orderidval;
 		$ins_query=Setting::insertqueryforthreefield($tbl_name,$request,$orderidval);
 		$actualId = "";
		$actualVal = Setting::selectOrderId($request);
		foreach ($actualVal as $key => $value) {
			$actualId .=  $value->orderId.",";
	...
	/**  
	*  Exist Check For Two textpopup
	*  @author Sarath
	*  @param $request
	*  Created At 2020/10/09
	**/
	
	public function getdataexisttwotextpopup(Request $request){
		$dateexistcheck = Setting::fnExistschktwotext($request);
		$dateexistcheck = count($dateexistcheck);
		echo $dateexistcheck;exit();
	}

	/**  
	*  Exist Check For Three textpopup
	*  @author Sarath
	*  @param $request
	*  Created At 2020/10/09
	**/

	public function getdataexistthreetextpopup(Request $request){
		$dateexistcheck = Setting::fnExistschkthreetext($request);
		$dateexistcheck = count($dateexistcheck);
		echo $dateexistcheck;exit();
	}
