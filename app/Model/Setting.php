...
class Setting extends Model { 
	...
	**/
	...
	/**  
	*  For house select for sold house
	*  @author Madasamy 
	*  @param $userId,$viewFlg
	...
	public static function fnUpdateHouseSold($request) {
		...
		$update = $db->table('hms_house_details')
					...
					->update(['soldFlg' => 1,
							  ...
							  'soldAmount' => str_replace(",", "", $request->soldAmount)]);
		return $update;
	}

	/**  
	*  Insert process For Three field popup
	*  @author sarath 
	*  @param $tbl_name,$request,$order_id
	*   Created At 2020/10/08
	**/

	public static function insertqueryforthreefield($tbl_name,$request,$orderid) {
		$db = DB::connection('mysql');
		$getTableFields = settingcommon::getDbFieldsforProcess();
		$DBType = $getTableFields[$tbl_name]['insertfields'][0];
		$DBType1 = $getTableFields[$tbl_name]['insertfields'][1];
		...
		$orderId = $getTableFields[$tbl_name]['insertfields'][8];
		$sql = $db->table($tbl_name)->insert([
					$DBType => $request->textbox1,
					$DBType1 => $request->textbox2,
					$DBType2 => $request->textbox3,
					$orderId => $orderid,
					$DelFlg =>'0',
					$InsDT => date('Y-m-d H-i-s'),
					$UpDT => date('Y-m-d H-i-s'),
	...
	/**  
	*  Update process For Three field popup
	*  @author sarath 
	*  @param $request
	*  Created At 2020/10/08
	**/
	...
	public static function updatethreeField($request) {
		...
		$Typename1 = $getTableFields[$request->tablename]['updatefields'][0];
		$Typename2 = $getTableFields[$request->tablename]['updatefields'][1];
		$Typename3 = $getTableFields[$request->tablename]['updatefields'][2];
		$UpDT = $getTableFields[$request->tablename]['updatefields'][3];
	...
	**/
	...
	public static function fnExistschktwotext($request) {
		...
		$result= $db->TABLE($request->tablename)
					->select($type1,$type2)
					->WHERE($type1, '=', $request->textbox1)
					->WHERE($type2, '=', $request->textbox2);
		if ($request->editid != 0) {
			$result = $result->WHERE('id', '!=', $request->editid);
		}
		$result = $result->get();
	...
	**/
	...
	public static function fnExistschkthreetext($request) {
		...
		$db = DB::connection('mysql');
		$result= $db->TABLE($request->tablename)
					->select($type1,$type2,$type2)
					->WHERE($type1, '=', $request->textbox1)
					->WHERE($type2, '=', $request->textbox2)
					->WHERE($type3, '=', $request->textbox3);
		if ($request->editid != 0) {
			$result = $result->WHERE('id', '!=', $request->editid);
		}
		$result = $result->get();
